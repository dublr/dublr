...
contract OmniToken is OmniTokenInternal {
     ...
     */
    function __mint(address operator, address account, uint256 amount, 
     ...
     */ 
    function __spendAllowance(address operator, address holder, uint256 amount)
            // Modified with `stateUpdater`, so `extCaller` functions cannot be called. This ensures that
            // future code changes do not break the Checks-Effects-Interactions pattern.
            private stateUpdater {
        ...
        if (allowedAmount != UNLIMITED_ALLOWANCE) {
            ...
            __setAllowance(holder, /* spender = */ operator, newAllowedAmount, expirationTimestamp);
        }
     ...
     * @param operatorData Data generated by the operator to be passed to the recipient.
     */
    function __burn(address operator, address account, uint256 amount,
     ...
     * @param operatorData Data generated by the operator to be passed to the recipient.
     */
    function __transfer(address operator, address holder, address recipient, uint256 amount, bool useAllowance,
        ...
        if (useAllowance && amount > 0) {
            ...
            __spendAllowance(operator, holder, amount);
        }
     ...
     */
    function __approve(address holder, address spender, uint256 allowedAmount, uint256 expirationTimestamp) internal {
        ...
        __setAllowance(holder, spender, allowedAmount, expirationTimestamp);
    ...
    function _mint(address operator, address account, uint256 amount, 
        ...
        __mint(operator, account, amount, data, operatorData);
    ...
    function _burn(address operator, address account, uint256 amount,
        ...
        __burn(operator, account, amount, data, operatorData);
    ...
    function _transfer(address operator, address holder, address recipient, uint256 amount, bool useAllowance,
        ...
        __transfer(operator, holder, recipient, amount, useAllowance, data, operatorData);
    ...
    function _approve(address holder, address spender, uint256 allowedAmount, uint256 expirationTimestamp,
        ...
        __approve(holder, spender, allowedAmount, expirationTimestamp);
