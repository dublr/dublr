...
describe("Dublr", () => {
  ...
  it("Minting without any sell orders", async () => {
    const contract1 = await contract0.connect(wallet[1]);
    await expect(contract1["buy()"]()).to.be.revertedWith("Zero payment");
    await contract1["buy()"]({value: 1});
    const amt0 = toDUBLR(1);
    expect(await contract1.balanceOf(wallet[1].address)).to.equal(amt0);
    await contract1["buy()"]({value: 5});
    ...
    await ethers.provider.send("evm_mine");
    await contract1["buy()"]({value: 1000000});
  ...
  it("Sell orders can be bought", async () => {
    ...
    await contract1["buy()"]({value: 10, gasPrice: 0});  // 10 ETH wei => 2M DUBLR wei
    await contract2["buy()"]({value: 10, gasPrice: 0});
    ...
    await contract3["buy()"]({value: 2, gasPrice: 0});
  ...
  it("Larger sell orders", async () => {
    ...
    // Mint coins for contract1 and contract2
    await contract1["buy()"]({value: 100000, gasPrice: 0});  // 100000 ETH wei => 20B DUBLR wei
    ...
    const ethBalance0_1 = await wallet[0].getBalance();
    await contract3["buy()"]({value: 2500, gasPrice: 0});
  ...
  it("Roll over from one sell order to the next when an order is exhausted", async () => {
    ...
    // Mint coins for contract1 and contract2
    await contract1["buy()"]({value: 100000, gasPrice: 0});  // 100000 ETH wei => 20B DUBLR wei
    await contract2["buy()"]({value: 100000, gasPrice: 0});
    expect(await contract0.balanceOf(wallet[1].address)).to.equal(2e10);
    ...
    await contract3["buy()"]({value: Math.ceil(3250 * 1.001), gasPrice: 0});
  ...
  it("Buying transitions from buying sell orders to minting at mint price", async () => {
    ...
    await contract2["buy()"]({value: amtSpent, gasPrice: 0});
  ...
  it("Mint price over 1.0 ETH per DUBLR", async () => {
    ...
    await contract1["buy()"]({value: amt0, gasPrice: 0});
    ...
    await contract1["buy()"]({value: amt1, gasPrice: 0});
    ...
    await contract1["buy()"]({value: amt2, gasPrice: 0});
  ...
  it("Unpayable seller", async () => {
    ...
    await contract0["buy()"]({value: 10});  // Mint 10 ETH worth => 2000000 DUBLR
    ...
    await expect(contract1["buy()"]({value: 10, gasPrice: 0}))
  ...
  it("Unpayable buyer", async () => {
    ...
    await expect(unpayableBuyer.buy(dublr.address, {value: 25})).to.be.revertedWith("Can't give buyer change");
  });
